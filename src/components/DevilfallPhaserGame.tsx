...
import { createDevilfallGame } from '../games/DevilfallPhaser';
...
export default function DevilfallPhaserGame({ onGameOver }: Props) {
  ...
  useEffect(() => {
    ...
    const baseHealth = 100 + levelIndex * 40;
    const baseDmg = 12 + levelIndex * 5;
    ...
    gameRef.current = createDevilfallGame({
  ...
  return (
    <div className="w-full flex flex-col items-center" style={{ fontFamily: '"Noto Serif SC", "Songti SC", "STSong", serif' }}>
      <div
        ref={wrapRef}
        className="rounded-2xl overflow-hidden border border-purple-700/30 shadow-[0_0_60px_rgba(168,85,247,0.12)] bg-slate-950 w-full max-w-[480px]"
